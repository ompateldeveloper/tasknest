...
export default function Task({ data }) {
    ...
    return (
        <motion.div layout  className="task-wrapper py-2 border-b-zinc-200 mx-4" style={{ borderBottomWidth:!layout? 1:0 }}>
            ...
            <div className={`flex  bg-zinc-100 h-12 items-center rounded-md overflow-hidden ${layout?" w-64":""}`}>
                <motion.div layout><div className={`circle h-2 w-2 ml-2 rounded-full ${data.completed?" saturate-0 ":" "}  ${priorityColor[data?.priority]} `} ></div></motion.div>
                <motion.div layout className={`title bg-transparent px-4 truncate mr-auto ${data.completed?"text-zinc-600 line-through":""}`}>
                ...
                <motion.div layout className="edit p-px rounded-md hover:shadow-lg border-transparent border-2 " onClick={() => { setModal(true) }}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                    </svg>
                </motion.div>
                <motion.div layout className="delete mr-4 ml-1 p-1 rounded-md hover:shadow-lg" >
                    {deleteQuestion ?
